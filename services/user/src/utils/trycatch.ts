// Wraps async Express handlers so controller errors are handled consistently.
...
const TryCatch = (handler: RequestHandler): RequestHandler => {
    return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            console.error(`[TryCatch Error] inside ${handler.name || 'anonymous function'}:`, error?.message || error);
